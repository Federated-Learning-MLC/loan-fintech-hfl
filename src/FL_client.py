...
import local_utility, config
from src.paths import DATA_DIR, CLIENTS_DATA_DIR
from config import EPOCHS, BATCH_SIZE, QUANTISATION, SMPC_NOISE, LEARNING_RATE
...
class BankFLClient(fl.client.NumpyClient):
    ...
    def fit(self, parameters: List[np.ndarray], config: Dict[str, str]) -> Tuple[List[np.ndarray], int,  Dict]:
        ...
        return self.get_parameters(config), 1, {'exposure': self.exposure}
...
def train(model, optimizer, criterion,
          ...
          val_loader: torch.utils.data.DataLoader, epochs=EPOCHS):
    """
...
def parse_args():
    ...
    Returns:
        ...
        - `client_id` (int): Specifies the partition of data for training. A value of -1 indicates that all data should be used ( training a global model ). Valid values are in the range [-1, run_config.server_config["num_clients"]].
    ...
    Example command line usage:
        ...
        `python script.py --client_id -1 --if_FL 1`

        For federated learning for client 3:
        `python script.py --client_id 3 --if_FL 1`
    """
    parser = argparse.ArgumentParser(description="Run federated or centralized training")
    parser.add_argument("--client_id", type=int, default=-1, choices=range(-1, 5),
...
def initialize_model():
    ...
    Note:
        - The function relies on global variables `device`, `config`, `optim`, and `nn` 
          for its operation.
        - `config` should have `NUM_FEATURES`, `NUM_UNITS_1`, and `NUM_UNITS_2` attributes defined.
        - `device` should be defined globally and indicate where the model should be allocated (e.g., 'cuda' or 'cpu').

    Returns:
        tuple: A tuple containing three elements:
            - model (): The initialized multiple regression model.
...
def main():
    ...
    train_dataset, val_dataset, test_dataset, _, _, exposure = local_utility.load_individual_data(args.client_id, config.IF_TRAIN_VAL)  # in folder data each training participant is storing their private, unique dataset 
