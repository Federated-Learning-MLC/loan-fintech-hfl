...
from src.quantization import modulus, M


def calc_noise(file_path: str, agent_number: int) -> dict:
    """
    Calculate noise based on seeds stored in a CSV file for a specific client.
    ...
    the run configuration. It differentiates the noise for the specific client from others.

    Parameters:
        file_path : str
            The path to the CSV file containing the seeds.
        agent_number : int
            The index of the current client for whom the noise is being calculated.
    ...
    for round_number, seeds_per_round in seeds.items():
        ...
        for i, seed in enumerate(seeds_per_round):
            ...
            if config.QUANTISATION:
                noise_vector = modulus(noise_vector + ((config.server_config["num_clients"] - 1) * noise_adjustment if i == agent_number else -noise_adjustment))
            else:
                uniform_noise = np.round(np.random.uniform(-2, 2, 162), 5)
                noise_vector = noise_vector.astype(float)
                noise_vector += uniform_noise * float(config.server_config["num_clients"] - 1 if i == agent_number else -1)
